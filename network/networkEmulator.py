...
class Topology(Topo):
    def build(self, **params):
        ...
        for i in range(1, hostNumber + 1):
            ...
            router_netmask = ip_to_bits(
                routers[router_name][interface_name]["mask"]
            ).count("1")
            self.addLink(
                switch,
                mininet_routers[router_name],
                intfName1=f"s1-eth{i}",
                intfName2=f"{router_name}-{interface_name}",
                param2={"ip": f"{router_ip}/{router_netmask}"},
                delay=DELAY,
                loss=LOSS,
            )
        ...
        for i in range(1, hostNumber + 1):
            ...
            self.addLink(
                mininet_routers[router_name],
                mininet_hosts[host_name],
                intfName1=f"{router_name}-{router_interface_name}",
                intfName2=f"{host_name}-{host_interface_name}",
            )
    ...
    # START THE MININET EMULATOR
        ...
        for host, interfaces in hosts.items():
            for interface, config in interfaces.items():
                net[host].cmd(
                    f'ifconfig {host}-{interface} {config["address"]} netmask {config["mask"]}'
                )
        ...
        for router, interfaces in routers.items():
            for interface, config in interfaces.items():
                net[router].cmd(
                    f'ifconfig {router}-{interface} {config["address"]} netmask {config["mask"]}'
                )
        ...
        for i in range(1, hostNumber + 1):
            ...
            net[host_name].cmd(
                f"ip route add default via {router_address} dev {host_name}-{host_interface}"
            )
        ...
        for i in range(4, hostNumber + 1):
            ...
            terminals.append(
                makeTerm(
                    host,
                    cmd=f"bash -c 'cd ../out/production/progetto1 && java peer.Peer 10.0.{i}.1 5000 10.0.{first_leader_number}.1 5000; exec bash'",
                )
            )
        h1 = net["h1"]
        terminals.append(
            makeTerm(
                h1,
                cmd=f"bash -c 'cd ../out/production/progetto1 && java client.Client 10.0.1.1 6000 10.0.{first_leader_number}.1 5000; exec bash'",
            )
        )
        h2 = net["h2"]
        terminals.append(
            makeTerm(
                h2,
                cmd=f"bash -c 'cd ../out/production/progetto1 && java client.Client 10.0.2.1 6000 10.0.{first_leader_number}.1 5000; exec bash'",
            )
        )
